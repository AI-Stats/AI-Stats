-- Keep catalogue-only model pages available when a provider placeholder exists.
-- Coming-soon/non-routable routes must not suppress the canonical model overview.
...
comment on function public.get_v2_model_overview(text, text, text) is
  'Returns stable catalogue facts for visible models, including models without active provider routes, plus any available route metadata.';
