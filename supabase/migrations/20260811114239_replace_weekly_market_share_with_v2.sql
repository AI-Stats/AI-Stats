...
begin
  ...
  top_groups as (
    ...
    order by sum(grouped.token_count) desc, grouped.group_name
  ...
  from bucketed
  order by bucketed.time_bucket, bucketed.token_count desc;
end;
$function$;
...
begin
  ...
  if exists (
    select 1
    from cron.job
    where command ~* 'public\.(public_usage_weekly_models|public_usage_weekly_providers|public_usage_weekly_organisations)\b'
  ) then
    raise exception 'A cron command still references a retired weekly usage table';
  end if;
end
$assert$;
