...
set
  ...
  slug = 'phaseo-task-v1',
  updated_at = now()
where kind = 'openrouter_task'
  and slug = 'openrouter-task-v1';

update public.public_model_task_daily
set taxonomy_slug = 'phaseo-task-v1'
where taxonomy_slug = 'openrouter-task-v1';

alter table public.workspace_classifiers
  add constraint workspace_classifiers_kind_check
  check (kind in ('phaseo_task', 'custom'));
