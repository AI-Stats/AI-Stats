-- Expose the provider execution regions already stored in v2_providers so the
-- public Providers page can filter by known datacenter coverage.
-- phaseo:allow-destructive-migration reason: Replaces the provider index and scoped admin mutation functions to add an existing provider metadata field.
...
as $$
  ...
  select
    ...
    coalesce(provider.default_execution_regions, array[]::text[]),
...
comment on function public.get_public_provider_index() is
  'Returns provider coverage rows, lifecycle status, BYOK, policy, and datacenter metadata for the cached public Web API provider index.';

-- Keep the internal provider editor able to maintain the existing execution-region metadata.
...
begin
  ...
  if p_resource_type = 'organisations' then
    ...
    if p_action='delete' then delete from public.v2_providers where provider_slug=p_resource_id;
      ...
      insert into public.v2_providers(provider_slug,name,status,country_code,base_url,default_execution_regions,byok_available,metadata,updated_at)
      values(p_resource_id,p_payload->>'api_provider_name',lower(coalesce(nullif(p_payload->>'status',''),'active')),coalesce(nullif(p_payload->>'country_code',''),'xx'),nullif(p_payload->>'link',''),case when jsonb_typeof(p_payload->'default_execution_regions') = 'array' then array(select jsonb_array_elements_text(p_payload->'default_execution_regions')) else null end,coalesce((p_payload->>'byok_available')::boolean,false),jsonb_strip_nulls(jsonb_build_object('description',p_payload->>'description','prompt_training_policy',p_payload->>'prompt_training_policy','prompt_training_notes',p_payload->>'prompt_training_notes','prompt_training_source_url',p_payload->>'prompt_training_source_url','data_policy_tier',p_payload->>'data_policy_tier','data_policy_confidence',p_payload->>'data_policy_confidence','data_policy_contract_mode',p_payload->>'data_policy_contract_mode','data_policy_contract_notes',p_payload->>'data_policy_contract_notes','source','admin')),now())
      on conflict(provider_slug) do update set name=excluded.name,status=excluded.status,country_code=excluded.country_code,base_url=excluded.base_url,default_execution_regions=case when p_payload ? 'default_execution_regions' then excluded.default_execution_regions else public.v2_providers.default_execution_regions end,byok_available=case when p_payload ? 'byok_available' then excluded.byok_available else public.v2_providers.byok_available end,metadata=public.v2_providers.metadata||excluded.metadata,updated_at=now();
    end if;
