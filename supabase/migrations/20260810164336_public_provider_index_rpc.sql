-- Build the public provider catalogue as one database aggregate. The Web API
-- remains the only public consumer and applies Cloudflare edge caching to the
-- result, so clients never page through raw provider-route rows.
...
comment on function public.get_public_provider_index() is
  'Returns provider coverage rows and an explicit gateway-routable flag for the cached public Web API provider index.';
