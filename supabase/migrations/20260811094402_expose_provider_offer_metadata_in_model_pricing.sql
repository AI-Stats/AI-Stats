-- Preserve provider-offer identity in the public model pricing projection so
-- regional and specialized prices remain distinguishable in every consumer.
...
as $$
  ...
  provider_models as (
    select
      ...
      provider.provider_family_slug,
      provider.offer_label,
      provider.offer_scope,
  ...
  grouped as (
    select
      model.provider_slug,
      model.provider_name,
      model.provider_family_slug,
      model.offer_label,
      model.offer_scope,
      model.country_code,
      model.default_execution_regions,
      model.default_data_regions,
  ...
  select jsonb_build_object(
    'provider', jsonb_build_object(
      ...
      'provider_family_id', grouped.provider_family_slug,
      'offer_label', grouped.offer_label,
      'offer_scope', grouped.offer_scope,
      'country_code', grouped.country_code,
      'default_execution_regions', grouped.default_execution_regions,
      'default_data_regions', grouped.default_data_regions,
