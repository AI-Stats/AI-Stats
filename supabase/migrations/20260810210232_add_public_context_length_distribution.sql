...
as $$
  ...
  select
    ...
    case
      when gate.request_count > 0
        then round(coalesce(counts.requests, 0)::numeric / gate.request_count::numeric * 100, 2)
      else 0
    end as share_percent,
    coalesce(counts.workspace_count, 0)::bigint as workspace_count
  from buckets bucket
  cross join privacy_gate gate
  left join counts on counts.bucket_key = bucket.bucket_key
  where gate.request_count >= greatest(coalesce(p_min_requests, 100), 1)
    and gate.workspace_count >= greatest(coalesce(p_min_workspaces, 3), 2)
...
comment on function public.get_public_context_length_distribution(integer, bigint, bigint) is
  'Privacy-gated public distribution of observed input-token lengths over a bounded recent period.';
