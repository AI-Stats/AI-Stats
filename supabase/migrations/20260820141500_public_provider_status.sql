-- Expose the provider-level lifecycle status to the public provider index so
-- catalogue-only providers can be distinguished from temporarily inactive ones.
...
comment on function public.get_public_provider_index() is
  'Returns provider coverage rows, lifecycle status, and an explicit gateway-routable flag for the cached public Web API provider index.';
