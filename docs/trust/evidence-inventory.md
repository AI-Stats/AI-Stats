...
| Database and authentication | Supabase clients in both applications; RLS migrations under `supabase/migrations`; production project inspected with Supabase CLI on 2026-08-30 | Supabase provides authentication and the primary relational data service; the production project is in AWS `eu-west-2` (London), has seven days of daily physical restore points, and does not have PITR enabled | Self-attested and production-verified | Recheck region and backup configuration after plan or project changes; do not imply every table has an identical policy |
...
| Private I/O logging | `apps/api/src/pipeline/audit/io-logging.ts`; `apps/api/src/pipeline/audit/io-retention-expiry.ts`; `apps/web/src/components/(gateway)/settings/privacy/PrivacySettingsClient.tsx`; I/O retention migrations | Feature-gated, workspace-controlled logging can store request, response, and optional provider payloads in private Cloudflare R2 for 90, 180, or 365 days. A five-minute scheduled job deletes expired R2 objects and their metadata index rows | Gated | The default workspace setting is off. Retention deletion failures must remain covered by operational monitoring |
...
| Account deletion | `apps/web/src/app/(dashboard)/settings/account/actions.ts`; `apps/api/src/pipeline/privacy/account-deletion.ts`; `supabase/migrations/20260830124329_account_deletion_workflow.sql`; focused API tests | After step-up authentication, the self-service workflow records owned workspace and key identifiers, deletes linked Stripe customer profiles, hard-deletes the Supabase Auth user so database cascades remove the public user and owned workspace data, and queues R2 and KV purges. The five-minute scheduled job retries failures, records deadline misses, and scrubs all user/workspace/key identifiers on completion | Implemented; deployment pending | Supabase backup restore points currently expire after seven days. Customer-directed recipients and legally required controller records follow their separate terms |
...
- Production regions and transfer mechanisms for Vercel, Upstash, Resend,
  Statsig, PostHog, Google, Notion, Tawk.to,
