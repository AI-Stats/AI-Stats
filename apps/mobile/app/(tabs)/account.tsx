...
export default function AccountScreen() {
  const { session, signOut } = useAuth(); const p = usePalette(); const [key, setKey] = useState(""); const [releaseAlerts, setReleaseAlerts] = useState(false);
  return <Screen><ScrollView contentContainerStyle={{ paddingBottom: 56 }}><Section title="Account"><Row icon={UserRound} title={session?.user.email ?? "Sign in"} subtitle={session ? "Profile, email and sign-in methods" : "Connect Google, GitHub, Apple or email"} /><Row icon={UsersRound} title="Workspace" subtitle="Members, roles and invitations" /></Section>
