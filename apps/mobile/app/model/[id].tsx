...
import { ActivityIndicator, Linking, ScrollView, Share, StyleSheet, Text } from "react-native";
...
export default function ModelDetail() {
  ...
  return <Screen safeTop={false}><ScrollView contentContainerStyle={{ paddingBottom: 48 }}><Text style={[styles.title, { color: p.text }]}>{m.name ?? m.id}</Text><Text selectable style={[styles.id, { color: p.muted }]}>{m.id}</Text>{m.status ? <Pill tone="blue">{m.status}</Pill> : null}<Text style={[styles.description, { color: p.muted }]}>{m.description ?? "No description is available for this model."}</Text>
    ...
    <Section title="Actions"><Row icon={Copy} title="Copy model ID" onPress={() => void Clipboard.setStringAsync(m.id)} /><Row icon={Share2} title="Share" onPress={() => void Share.share({ message: `https://phaseo.app/models/${m.id}` })} /><Row icon={ExternalLink} title="Open on Phaseo" onPress={() => void Linking.openURL(`https://phaseo.app/models/${m.id}`)} /></Section>
  </ScrollView></Screen>;
}
