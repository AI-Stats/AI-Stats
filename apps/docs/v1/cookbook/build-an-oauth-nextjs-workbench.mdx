...
**Goal:** Run the signed-in workbench and send Phaseo requests through one protected server-side proxy.

**Outcome:** A local application with OAuth sign-in, session-backed tokens, model discovery, chat, and an endpoint tester.
...
- an OAuth client configured with the local callback URL
...
- Sign-in returns to the configured callback and creates a session.
- The dashboard can discover models through the proxy.
- One Responses API request completes without exposing an access token to the browser.
- An endpoint outside the proxy allowlist is rejected.
