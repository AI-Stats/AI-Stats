...
Create a workspace-managed webhook endpoint first and store the signing secret returned by that management operation. Video jobs reference only the endpoint ID; Phaseo keeps the secret encrypted and supports rotation without changing job requests.
...
Persist and poll with the gateway-owned video `id`. `native_video_id`, when present, is provider-native correlation metadata; do not use it for Phaseo polling, WebSocket, or content URLs.
...
Webhook subscriptions accept generic `job.*` events or matching `video.*` events. If you omit `events`, Phaseo subscribes the webhook to `job.status_changed` plus terminal `job.completed`, `job.failed`, `job.cancelled`, and `job.expired` notifications. A status-change payload includes `status_change.previous_status` and `status_change.status`. If you provide `events`, at least one event must be valid for video jobs; all-invalid or cross-kind-only lists such as `["batch.completed"]` are rejected instead of being broadened silently.
