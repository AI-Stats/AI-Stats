...
Support differs by model. For example, Veo 3.1 supports first and last frames,
reference images, and video extension on its full routes; Veo Lite supports the
simpler first-frame flow. Seedance 2.x and Grok Imagine Video expose broader
reference workflows on their supported provider routes. Treat the catalogue
returned by `GET /v1/videos/models` as the current source of truth.
...
- [Generate video from audio](./generate-video-from-audio.mdx)
