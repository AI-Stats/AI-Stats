...
- reference images for style, character, or scene guidance
- source video for extension or editing
- source audio for audio-driven video
...
For media inputs, inspect `input_types`, `supported_params`, and the model's
capability modes together. An `image` input does not necessarily mean that a
model accepts multiple reference images, and generated audio is different from
accepting source audio.
