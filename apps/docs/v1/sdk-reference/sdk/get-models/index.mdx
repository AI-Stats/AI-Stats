...
- The default catalogue is public and only includes currently gateway-routable models. No API key is required.
- Use an API key with `availability=all` when you need non-routable availability records. Invalid credentials never fall back to anonymous access.
- `availability.status`, `availability.active_provider_count`, and each offer's `routable` field distinguish catalogue metadata from current gateway routing availability.
