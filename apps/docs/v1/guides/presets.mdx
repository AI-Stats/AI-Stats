...
Invoke a preset only through the request's `model` field using `@{slug}`. Phaseo does not expose a separate `preset` request field. Slugs are normalised to lower case and support letters, numbers, hyphens, underscores, periods, and colons.
