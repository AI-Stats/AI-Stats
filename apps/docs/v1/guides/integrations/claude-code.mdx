...
Setup creates a non-expiring `Phaseo CLI: Claude Code API Key`, stores it in the OS credential store, and configures Claude Code's gateway and API-key helper. No plaintext key is written to Claude Code settings.
