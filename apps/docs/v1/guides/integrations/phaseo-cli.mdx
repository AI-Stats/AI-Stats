...
title: "Phaseo CLI"
icon: "terminal"
description: "Install the official Phaseo CLI, connect coding harnesses, and manage Phaseo from the terminal."
---
...
Interactive terminals receive Phaseo's colour, hierarchy, and ephemeral setup spinners. Spinners use `stderr` and clear their line when work completes. Pipes, redirected output, `TERM=dumb`, and `NO_COLOR` receive deterministic plain text without animation. Commands using `--json` always return decoration-free structured output for agents and automation. Use `FORCE_COLOR=1` only when a terminal wrapper supports colour but does not report itself as a TTY.
