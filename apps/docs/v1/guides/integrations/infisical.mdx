...
- `me:read` to validate the connection
- `keys:read` to inspect managed keys
...
Phaseo accepts the OpenRouter-style compatibility fields `limit`, `limit_reset`, and `include_byok_in_limit`. Supported reset periods are daily, weekly, and monthly. `include_byok_in_limit` is currently compatibility-only; Phaseo excludes BYOK usage from this key-limit field.
