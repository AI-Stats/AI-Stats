...
## Set up with Phaseo CLI

Sign in once, then run setup for the assistant you use:

```bash
phaseo login
phaseo integrations setup <integration>
```
...
```bash
phaseo integrations setup <integration> --dry-run
phaseo integrations remove <integration>
```
...
## Pi, OpenClaw, and Hermes Agent

`phaseo integrations setup pi` installs a dedicated Pi extension that registers Phaseo and resolves its key from the Phaseo CLI at runtime. `phaseo integrations setup openclaw` registers an OpenClaw custom provider plus an exec-backed SecretRef, without placing the API key in `openclaw.json`. Both setups are removed automatically by their matching `phaseo integrations remove` command.

`phaseo integrations setup hermes` automatically configures the current Nous Research coding assistant. It saves the previous Hermes model settings, configures Phaseo as the custom endpoint, and uses Hermes' official `hermes config set` command to place the dedicated key in `~/.hermes/.env` as `OPENAI_API_KEY`. Hermes deliberately uses this plaintext file for provider credentials; protect it like any other API-key file. Removal restores the previous model settings, removes `OPENAI_API_KEY`, deletes Phaseo's recovery state, and revokes the key.
