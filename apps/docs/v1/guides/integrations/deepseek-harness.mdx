---
title: "Use Phaseo with DeepSeek Harness"
icon: "terminal"
...
- Phaseo CLI and a Phaseo account
...
The CLI installs DeepSeek Harness when needed, preserves unrelated entries in `$DSH_HOME/cordis.patch.yml`, and marks the Phaseo block so it can update or remove only the settings it owns. It syncs every active Phaseo text model compatible with Chat Completions. Use a different default model when needed:

```bash
phaseo dsh --model YOUR_PHASEO_MODEL_ID
```

Setup also creates a non-expiring gateway key named `Phaseo CLI: DeepSeek Harness API Key` and writes it directly to Harness's protected `$DSH_HOME/.credentials.yaml` store. Use `--catalog default` to configure only the selected model, or `--skip-install` when Harness is managed separately.
