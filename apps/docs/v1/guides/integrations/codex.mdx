...
Setup installs Codex when needed, creates a non-expiring `Phaseo CLI: Codex API Key`, stores it in the OS credential store, and writes a dedicated Phaseo profile. Codex retrieves the key through the CLI helper, so no plaintext key is written to `config.toml`.
