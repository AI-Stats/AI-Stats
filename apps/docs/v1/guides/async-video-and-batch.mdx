...
After an Anthropic batch reaches a terminal state, its `results_url` points to an authenticated Phaseo download. Use your normal Phaseo API key from the workspace that owns the batch:
...
The response streams Anthropic's original JSONL. Match rows using `custom_id`; successful rows contain the generated message in `result.message`. Downloading results does not submit another batch or add an inference charge. You do not need an Anthropic API key. Save results before the provider's retention period ends.

This download endpoint currently supports Anthropic. Other providers retain their existing output mechanisms, including `output_file_id` where supplied. The batch request-row endpoint contains tracking and billing metadata, not generated message bodies.
