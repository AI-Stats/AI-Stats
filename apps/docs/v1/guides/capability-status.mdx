...
| Capability | Status | Current contract |
| --- | --- | --- |
| Guardrail creation and enforced policies | Beta | Dashboard and management API paths enforce supported privacy, routing, model-restriction, and sensitive-info policies. |
| Guardrail assignment to keys and members | Beta | Management API and dashboard assignment paths are available. |
| Guardrail prompt-injection actions | Unavailable | Policy fields can be stored, but assigned guardrails do not activate prompt-injection enforcement in the request pipeline. |
| Guardrail request and cost budgets | Unavailable | Policy fields can be stored, but assigned guardrails do not apply these limits during request admission; API-key limits remain the enforced path. |
