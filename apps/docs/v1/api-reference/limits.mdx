...
Phaseo forwards a validated `Retry-After` value when the routed provider supplies one. For BYOK requests, provider quota details may also be returned under `X-Phaseo-Upstream-RateLimit-*` headers. Those quota headers are intentionally omitted for managed-provider requests because they describe shared Phaseo provider capacity, not an individual customer's allowance. Other upstream headers are not forwarded.
