...
| `region` | `string` | Require both execution and data handling in a region, for example `eu`. |
...
`region` is the concise form for the common case. For example, `region: "eu"` applies
EU execution and EU data-handling constraints. Use the two `required_*_region` fields
