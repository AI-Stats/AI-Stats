...
The selected endpoint defines the request-body shape for every row. Rows may
omit `url` to inherit the batch endpoint. If a row supplies `url`, it must
