...
export async function createSessionWithHold(args: {
	...
	return withDatabase((db) => db.transaction(async (tx) => {
		...
		await tx.execute(sql`insert into gateway_requests(workspace_id,request_id,realtime_session_id,endpoint,model_id,provider,stream,byok,status_code,success,usage,cost_nanos,currency,pricing_lines,key_id,created_at)
...
export async function findRequestSummary(sessionId: string, startedAt: string): Promise<{ id: string; created_at: string } | null> {
	return withDatabase(async (db) => (await db.execute<{ id: string; created_at: string }>(sql`select id,created_at from gateway_requests where realtime_session_id=${sessionId} and created_at=${startedAt}::timestamptz limit 1`))[0] ?? null);
}
