...
describe("SCIM discovery", () => {
	it("requires bearer authentication", async () => {
		...
		const response = await app.request("https://phaseo.app/ServiceProviderConfig", {}, env);
	...
	it("fails closed without the production rate-limit binding", async () => {
		const response = await authorized.request("https://phaseo.app/ServiceProviderConfig", {}, { ENV: "production" });
		expect(response.status).toBe(503);
	});

	it("returns a SCIM rate-limit error with retry guidance", async () => {
		const response = await authorized.request("https://phaseo.app/ServiceProviderConfig", {}, { ENV: "production", SCIM_RATE_LIMITER: { limit: async () => ({ success: false }) } });
	...
	it("advertises PATCH, filtering, and RFC bulk support", async () => {
		const response = await authorized.request("https://phaseo.app/ServiceProviderConfig", {}, env);
	...
	it("publishes User and Group resource types", async () => {
		const response = await authorized.request("https://phaseo.app/ResourceTypes", {}, env);
	...
	it("returns an individual schema by its URN", async () => {
		const response = await authorized.request(`https://phaseo.app/Schemas/${SCIM_URNS.enterpriseUser}`, {}, env);
		expect(response.status).toBe(200);
	...
	it("discovers all accepted core attributes and the enterprise manager", async () => {
		const response = await authorized.request("https://phaseo.app/Schemas", {}, env);
