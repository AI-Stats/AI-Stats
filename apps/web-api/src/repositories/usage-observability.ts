...
export async function getUsageMetadata(env: Env, args: { models?: string[]; providers?: string[]; apps?: string[] }) {
	...
	const {db,client}=createDatabase(env);try{
		const models=modelIds.length?await db.select({modelSlug:v2Models.modelSlug,labSlug:v2Models.labSlug,modelName:v2Models.name,labName:v2Labs.name,labColour:sql<string|null>`${v2Labs.metadata}->>'colour'`}).from(v2Models).leftJoin(v2Labs,eq(v2Labs.labSlug,v2Models.labSlug)).where(inArray(v2Models.modelSlug,modelIds)):[];
		const mappings=modelIds.length?await db.select({apiModelId:v2ModelProviderRoutes.providerModelSlug,modelId:v2ModelProviderRoutes.modelSlug}).from(v2ModelProviderRoutes).where(inArray(v2ModelProviderRoutes.modelSlug,modelIds)):[];
		const providers=providerIds.length?await db.select({providerSlug:v2Providers.providerSlug,name:v2Providers.name,colour:sql<string|null>`${v2Providers.metadata}->>'colour'`,providerFamilySlug:v2Providers.providerFamilySlug,offerLabel:v2Providers.offerLabel,offerScope:v2Providers.offerScope,promptTrainingPolicy:v2Providers.promptTrainingPolicy}).from(v2Providers).where(inArray(v2Providers.providerSlug,providerIds)):[];
		const apps=appIds.length?await db.select({id:apiApps.id,title:apiApps.title,appKey:apiApps.appKey,imageUrl:apiApps.imageUrl}).from(apiApps).where(inArray(apiApps.id,appIds)):[];
		const modelMetadata=new Map<string,Record<string,unknown>>();for(const model of models)modelMetadata.set(model.modelSlug,{organisationId:model.labSlug,organisationName:model.labName??model.labSlug,organisationColour:model.labColour,modelName:model.modelName});for(const mapping of mappings){const value=modelMetadata.get(mapping.modelId);if(value)modelMetadata.set(mapping.apiModelId,value);}
		const providerNames=new Map<string,string>();const providerMetadata=new Map<string,Record<string,unknown>>();for(const provider of providers){providerNames.set(provider.providerSlug,provider.name);providerMetadata.set(provider.providerSlug,{id:provider.providerSlug,name:provider.name,colour:provider.colour,providerFamilyId:provider.providerFamilySlug,offerLabel:provider.offerLabel,offerScope:provider.offerScope,promptTrainingPolicy:provider.promptTrainingPolicy});}
...
export async function loadUsageRequestPage(env:Env,input:{workspaceId:string;from:string;to:string;limit:number;cursor?:{createdAt:string;id:string}|null;stringFilters:Array<{column:RequestStringColumn;value:string;negate:boolean}>;success?:{value:boolean;negate:boolean};stream?:{value:boolean;negate:boolean};tokenFilters:Array<{column:"input"|"output"|"total";operator:"eq"|"lte"|"gte"|"between";value:number;max?:number}>}){
	const{db,client}=createDatabase(env);try{const columns={model:gatewayRequests.modelId,provider:gatewayRequests.provider,app:gatewayRequests.appId,endpoint:gatewayRequests.endpoint,finishReason:gatewayRequests.finishReason,errorCode:gatewayRequests.errorCode,statusCode:gatewayRequests.statusCode,key:gatewayRequests.keyId,requestId:gatewayRequests.requestId,session:gatewayRequests.sessionId,source:gatewayRequests.clientSourceId} as const;const conditions:any[]=[eq(gatewayRequests.workspaceId,input.workspaceId),gte(gatewayRequests.createdAt,input.from),lte(gatewayRequests.createdAt,input.to),sql`${gatewayRequests.endpoint} not in ('video.generation','batch','music.generate')`];for(const filter of input.stringFilters){const column=columns[filter.column];conditions.push(filter.negate?ne(column as any,filter.value as any):eq(column as any,filter.value as any));}if(input.success)conditions.push(input.success.negate?ne(gatewayRequests.success,input.success.value):eq(gatewayRequests.success,input.success.value));if(input.stream)conditions.push(input.stream.negate?ne(gatewayRequests.stream,input.stream.value):eq(gatewayRequests.stream,input.stream.value));const tokenColumns={input:gatewayRequests.usageInputTokens,output:gatewayRequests.usageOutputTokens,total:gatewayRequests.usageTotalTokens};for(const filter of input.tokenFilters){const column=tokenColumns[filter.column];if(filter.operator==="eq")conditions.push(eq(column,filter.value));else if(filter.operator==="lte")conditions.push(lte(column,filter.value));else if(filter.operator==="between"&&filter.max!=null)conditions.push(and(gte(column,filter.value),lte(column,filter.max)));else conditions.push(gte(column,filter.value));}
		if(input.cursor&&Number.isFinite(Date.parse(input.cursor.createdAt))&&input.cursor.id)conditions.push(sql`(${gatewayRequests.createdAt},${gatewayRequests.id})<(${input.cursor.createdAt}::timestamptz,${input.cursor.id}::uuid)`);
		const rows=await db.select({id:gatewayRequests.id,requestId:gatewayRequests.requestId,createdAt:gatewayRequests.createdAt,endpoint:gatewayRequests.endpoint,modelId:gatewayRequests.modelId,requestedModelId:gatewayRequests.requestedModelId,routedModelId:gatewayRequests.routedModelId,provider:gatewayRequests.provider,nativeResponseId:gatewayRequests.nativeResponseId,stream:gatewayRequests.stream,sessionId:gatewayRequests.sessionId,appId:gatewayRequests.appId,usage:gatewayRequests.usage,usageInputTokens:gatewayRequests.usageInputTokens,usageOutputTokens:gatewayRequests.usageOutputTokens,usageTotalTokens:gatewayRequests.usageTotalTokens,costNanos:gatewayRequests.costNanos,generationMs:gatewayRequests.generationMs,latencyMs:gatewayRequests.latencyMs,finishReason:gatewayRequests.finishReason,success:gatewayRequests.success,statusCode:gatewayRequests.statusCode,errorCode:gatewayRequests.errorCode,clientSourceId:gatewayRequests.clientSourceId,clientSourceName:gatewayRequests.clientSourceName,clientSourceKind:gatewayRequests.clientSourceKind,clientSourceVersion:gatewayRequests.clientSourceVersion,clientSourceDetection:gatewayRequests.clientSourceDetection,keyId:gatewayRequests.keyId,throughput:gatewayRequests.throughput}).from(gatewayRequests).where(and(...conditions)).orderBy(desc(gatewayRequests.createdAt),desc(gatewayRequests.id)).limit(input.limit+1);return rows.map((row)=>({id:row.id,request_id:row.requestId,created_at:row.createdAt,endpoint:row.endpoint,model_id:row.modelId,requested_model_id:row.requestedModelId,routed_model_id:row.routedModelId,provider:row.provider,native_response_id:row.nativeResponseId,stream:row.stream,session_id:row.sessionId,app_id:row.appId,usage:row.usage,usage_input_tokens:row.usageInputTokens,usage_output_tokens:row.usageOutputTokens,usage_total_tokens:row.usageTotalTokens,cost_nanos:row.costNanos,generation_ms:row.generationMs,latency_ms:row.latencyMs,finish_reason:row.finishReason,success:row.success,status_code:row.statusCode,error_code:row.errorCode,client_source_id:row.clientSourceId,client_source_name:row.clientSourceName,client_source_kind:row.clientSourceKind,client_source_version:row.clientSourceVersion,client_source_detection:row.clientSourceDetection,key_id:row.keyId,throughput:row.throughput}));
	}finally{await client.end({timeout:1});}
}
