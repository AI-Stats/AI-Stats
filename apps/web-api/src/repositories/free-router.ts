	...
	try {
		const result = await db.execute<Record<string, unknown>>(sql`
			with usage as (
				...
				from v2_request_facts fact left join v2_request_pricing_lines pricing using(request_event_id)
			...
			from v2_models model join v2_model_provider_routes route on route.model_slug=model.model_slug
			left join v2_labs lab on lab.lab_slug=model.lab_slug left join usage on usage.model_slug=model.model_slug
