...
export function listModelPerformance(env: Env, hours: number) {
	return rows(env, sql`
		...
		from v2_public_usage_hourly usage
		left join v2_model_provider_routes route on route.provider_model_id = usage.provider_model_id
...
export async function listFastestModels(env: Env, days: number, limit: number) {
	const result = await rows(env, sql`
		select usage.model_slug model_id, coalesce(route.provider_slug, 'unknown') provider,
			...
			round(sum(usage.throughput_sum)::numeric / nullif(sum(usage.throughput_count), 0), 2) median_throughput,
			round(sum(usage.successful_requests)::numeric / nullif(sum(usage.requests), 0), 4) success_rate
		from v2_public_usage_daily usage
		left join v2_model_provider_routes route on route.provider_model_id = usage.provider_model_id
...
export function listUsageTimeseries(env: Env, timeRange: string, bucketSize: string, topN: number) {
	...
	return rows(env, sql`
		with meter as (
			...
			from v2_public_usage_hourly_meters group by rollup_id
		), base as (
			...
			from v2_public_usage_hourly usage left join meter using (rollup_id)
		...
		from ranked left join v2_models model on model.model_slug=ranked.model_id
		left join v2_labs lab on lab.lab_slug=model.lab_slug
...
export function listModalityTimeseries(env: Env, metric: string, timeRange: string, topN: number) {
	...
	return rows(env, sql`
		with base as (
			...
			from v2_public_usage_daily usage join v2_public_usage_daily_meters meter using (rollup_id)
...
export function listUniqueUsers(env: Env, timeRange: string, bucketSize: string, topN: number) {
	...
	return rows(env, sql`
		with base as (
			...
			from public_model_user_usage_daily usage
...
export function listIntelligenceIndex(env: Env, limit: number) {
	return rows(env, sql`
		with scored as (
			...
			from v2_benchmark_results result join v2_benchmarks benchmark using (benchmark_id)
			join v2_models model using (model_slug) left join v2_labs lab on lab.lab_slug=model.lab_slug
...
export function listModelRankings(env: Env, timeRange: string, metric: string, limit: number) {
	...
	return rows(env, sql`
		with meters as (
			...
			from v2_public_usage_daily_meters group by rollup_id
		), base as (
			select usage.model_slug model_id, coalesce(route.provider_slug,'unknown') provider,
				...
				round(sum(usage.throughput_sum)::numeric/nullif(sum(usage.throughput_count),0),2) median_throughput,
				round(sum(usage.successful_requests)::numeric/nullif(sum(usage.requests),0),4) success_rate
			from v2_public_usage_daily usage left join meters using(rollup_id)
			left join v2_model_provider_routes route on route.provider_model_id=usage.provider_model_id
			where usage.usage_date >= current_date-${days} and lower(usage.model_slug) not in ('unknown','other')
			group by usage.model_slug,coalesce(route.provider_slug,'unknown')
...
export function listTrendingModels(env: Env, limit: number) {
	return rows(env, sql`
		with weekly as (
			...
			from v2_public_usage_daily usage left join v2_model_provider_routes route on route.provider_model_id=usage.provider_model_id
...
export function getRankingSummary(env: Env) {
	return rows(env, sql`
		with meters as (select rollup_id,sum(quantity) filter(where meter_key in ('input_tokens','output_tokens')) tokens from v2_public_usage_hourly_meters group by rollup_id)
		...
		from v2_public_usage_hourly usage left join meters using(rollup_id)
		left join v2_model_provider_routes route on route.provider_model_id=usage.provider_model_id
...
export function listMarketShare(env: Env, dimension: "provider" | "organization", timeRange: string) {
	...
	return rows(env, sql`
		with meters as (select rollup_id,sum(quantity) filter(where meter_key in ('input_tokens','output_tokens')) tokens from v2_public_usage_daily_meters group by rollup_id),
		base as (
			...
			from v2_public_usage_daily usage left join meters using(rollup_id)
			left join v2_model_provider_routes route on route.provider_model_id=usage.provider_model_id
			left join v2_providers provider on provider.provider_slug=route.provider_slug
			left join v2_models model on model.model_slug=usage.model_slug left join v2_labs lab on lab.lab_slug=model.lab_slug
...
export function listMarketShareTimeseries(env: Env, dimension: "provider" | "organization", timeRange: string, bucketSize: string, topN: number) {
	...
	return rows(env, sql`
		...
		base as (
			...
			from v2_public_usage_daily usage left join meters using(rollup_id)
			left join v2_model_provider_routes route on route.provider_model_id=usage.provider_model_id
			left join v2_models model on model.model_slug=usage.model_slug
...
export function listGeography(env: Env, from: Date, to: Date) {
	return rows(env, sql`
		with facts as materialized(select request_event_id,workspace_id,edge_country from v2_request_facts where occurred_at>=${from.toISOString()} and occurred_at<${to.toISOString()} and edge_country is not null),
		tokens as(select fact.request_event_id,coalesce(nullif(sum(usage.quantity) filter(where usage.meter_key in ('input_tokens','output_tokens')),0),sum(usage.quantity) filter(where usage.meter_key in ('input_text_tokens','output_text_tokens','input_image_tokens','output_image_tokens','input_audio_tokens','output_audio_tokens','input_video_tokens','output_video_tokens')),0) tokens from facts fact left join v2_request_usage usage using(request_event_id) group by fact.request_event_id),
...
export function listContextLengths(env: Env, days: number) {
	return rows(env, sql`
		with scoped as materialized(select request_event_id,workspace_id from v2_request_facts where occurred_at>=now()-(${days}*interval '1 day')),
		per_request as(select scoped.request_event_id,scoped.workspace_id,coalesce(sum(usage.quantity) filter(where usage.meter_key in ('input_tokens','input_text_tokens','prompt_tokens')),0)::bigint input_tokens from scoped left join v2_request_usage usage using(request_event_id) group by 1,2),
...
export async function loadRankingBenchmarks(env: Env, benchmarkIds: readonly string[]) {
	...
	try {
		...
		const [benchmarks, scores] = await Promise.all([
			db.execute<RankingRow>(sql`select benchmark_id,name,category,ascending_order,benchmark_type,total_models from v2_benchmarks where benchmark_id in (${ids})`),
			db.execute<RankingRow>(sql`select benchmark_id,model_slug,score_numeric,rank from v2_benchmark_results where benchmark_id in (${ids}) and score_numeric is not null limit 2000`),
		]);
		const modelIds = [...new Set([...scores].map((row) => String(row.model_slug ?? "")).filter(Boolean))];
		const models = modelIds.length
			? await db.execute<RankingRow>(sql`select model.model_slug,model.name,model.lab_slug,lab.name lab_name from v2_models model left join v2_labs lab on lab.lab_slug=model.lab_slug where model.model_slug in (${sql.join(modelIds.map((id) => sql`${id}`), sql`, `)}) and model.hidden=false`)
			: [];
...
export function listProviderMetadata(env: Env, ids: string[]) {
	return rows(env, sql`select provider_slug,name,metadata->>'colour' colour from v2_providers where provider_slug in (${sql.join(ids.map((id) => sql`${id}`), sql`, `)})`);
}

export function listOrganisationLogoIds(env: Env, names: string[]) {
	return rows(env, sql`select lab_slug,name from v2_labs where lab_slug in (${sql.join(names.map((id) => sql`${id}`), sql`, `)}) or name in (${sql.join(names.map((id) => sql`${id}`), sql`, `)})`);
}
...
export function listModelMetadata(env: Env, ids: string[]) {
	...
	return rows(env, sql`
		with requested(requested_id) as (values ${values}), resolved as (
			...
			from requested left join v2_models direct on direct.model_slug=requested.requested_id and direct.hidden=false
			left join lateral(select model_slug from v2_model_provider_routes where provider_model_id=requested.requested_id limit 1) route_id on direct.model_slug is null
			left join lateral(select model_slug from v2_model_provider_routes where provider_model_slug=requested.requested_id limit 1) route_slug on direct.model_slug is null and route_id.model_slug is null
			left join v2_model_aliases alias on alias.alias_slug=requested.requested_id and alias.enabled=true
		)
		...
		from resolved left join v2_models model on model.model_slug=resolved.model_slug and model.hidden=false
		left join v2_labs lab on lab.lab_slug=model.lab_slug
	`);
}
