...
export async function listProviderTopModels(env: Env, providerId: string, since: string, limit: number) { const { db, client } = createDatabase(env); try { const rows = await db.execute<Record<string, unknown>>(sql`select coalesce(nullif(request.canonical_model_id,''),request.model_id) model_id, max(model.name) model_name, count(*)::bigint request_count, sum(coalesce(request.usage_total_tokens,0))::bigint total_tokens, percentile_cont(.5) within group(order by request.latency_ms) filter(where request.latency_ms>0) median_latency_ms, percentile_cont(.5) within group(order by request.throughput) filter(where request.throughput>0) median_throughput from ${gatewayRequests} request join ${v2Models} model on model.model_slug=coalesce(nullif(request.canonical_model_id,''),request.model_id) and model.hidden=false where request.provider=${providerId} and request.created_at>=${since}::timestamptz group by 1 order by total_tokens desc limit ${limit}`); return [...rows]; } finally { await client.end({ timeout: 1 }); } }

export async function listProviderTopApps(env: Env, providerId: string, since: string, limit: number) { const { db, client } = createDatabase(env); try { const rows = await db.select({ app_id: apiApps.id, title: apiApps.title, url: apiApps.url, image_url: apiApps.imageUrl, total_tokens: sql<string>`sum(${v2WebPublicUsageHourly.totalTokens})` }).from(v2WebPublicUsageHourly).innerJoin(apiApps, and(eq(apiApps.id, v2WebPublicUsageHourly.appId), eq(apiApps.isPublic, true))).where(and(eq(v2WebPublicUsageHourly.provider, providerId), gte(v2WebPublicUsageHourly.bucket15M, since))).groupBy(apiApps.id, apiApps.title, apiApps.url, apiApps.imageUrl).orderBy(desc(sql`sum(${v2WebPublicUsageHourly.totalTokens})`)).limit(limit); return rows; } finally { await client.end({ timeout: 1 }); } }

export async function getProviderRecentTokens(env: Env, providerId: string, since: string) { const { db, client } = createDatabase(env); try { const [row] = await db.select({ tokens: sql<string>`coalesce(sum(${v2WebPublicUsageHourly.totalTokens}),0)` }).from(v2WebPublicUsageHourly).where(and(eq(v2WebPublicUsageHourly.provider, providerId), gte(v2WebPublicUsageHourly.bucket15M, since))); return Number(row?.tokens ?? 0); } finally { await client.end({ timeout: 1 }); } }
