		...
		<div className="space-y-6">
			<section className="space-y-5">
				...
				<div className="grid gap-6 border-b border-border/60 pb-5 sm:grid-cols-2">
					<div><h3 className="text-sm font-semibold">{enterprise("subscription")}</h3><dl className="mt-3 space-y-2 text-sm"><div className="flex justify-between gap-4"><dt className="text-muted-foreground">{enterprise("plan")}</dt><dd>{enterprise("selfServePlan")}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted-foreground">{enterprise("membersIncluded")}</dt><dd>{summary?.includedMembers?.toLocaleString("en-US") ?? "—"}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted-foreground">{enterprise("creditTopUpFee")}</dt><dd>{summary?.feePolicy === "included_allowance" ? enterprise("includedAllowance") : "5% ($1 minimum)"}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted-foreground">{enterprise("renewal")}</dt><dd>{summary?.grandfathered ? enterprise("includedAllowance") : periodEnd ?? enterprise("active")}</dd></div></dl></div>
