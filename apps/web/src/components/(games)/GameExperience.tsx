...
function ModelPicker({
  ...
  const selected = available.find(
    (candidate) =>
      candidate.id.toLowerCase() === value.trim().toLowerCase() ||
      `${candidate.name} — ${candidate.labName}`.toLowerCase() ===
  ...
  return (
    ...
    >
      <div className="min-w-0 flex-1">
        <label className="sr-only" htmlFor="model-guess">
          Model
        </label>
        <Input
          id="model-guess"
          list="model-guess-options"
          value={value}
          ...
          className="h-10"
        />
        <datalist id="model-guess-options">
          {available.map((candidate) => (
            <option
              key={candidate.id}
              value={`${candidate.name} — ${candidate.labName}`}
            />
          ))}
        </datalist>
      </div>
