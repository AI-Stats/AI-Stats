...
export async function generateMetadata({ params }: { params: Promise<{ locale: PublicLocale }> }) {
	...
	const t = await getTranslations({ locale, namespace: "Content.contribute" });
	return buildMetadata({ title: t("title"), description: t("description"), path: "/contribute", keywords: ["Phaseo contributors", "AI sponsors", "open source contributors"] });
}
...
export default async function Page({ params }: { params: Promise<{ locale: PublicLocale }> }) {
	const { locale } = await params;
	const t = await getTranslations({ locale, namespace: "Content.contribute" });
					...
					<CardHeader className="space-y-2">
						<div className="flex items-center justify-between gap-4">
							...
							<Badge variant="secondary" className="shrink-0">
								{t("contributorCount", { count: sortedContributors.length })}
							</Badge>
						</div>
					</CardHeader>
				...
				<Card className="h-full">
					<CardHeader className="space-y-2">
						<div className="flex items-center justify-between gap-4">
							...
							<Badge variant="secondary" className="shrink-0">
							{t("listed", { count: sponsors.length })}
							</Badge>
						</div>
					</CardHeader>
					<CardContent className="pt-0">
