...
function FeedbackCTA({ feedback, cta }: { feedback: string; cta: string }) {
	return (
...
export async function generateMetadata({ params }: { params: Promise<{ locale: PublicLocale }> }): Promise<Metadata> {
	...
	const t = await getTranslations({ locale, namespace: "Content.roadmap" });
	return { title: t("title"), description: t("description"), keywords: ["Phaseo roadmap", "product roadmap", "AI gateway roadmap"], alternates: { canonical: "/roadmap" } };
}

export default async function RoadmapPage({ params }: { params: Promise<{ locale: PublicLocale }> }) {
	const { locale } = await params;
	const t = await getTranslations({ locale, namespace: "Content.roadmap" });
		...
		<main className="min-h-screen">
			<section className="mx-auto container px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
				<div className="mx-auto max-w-3xl text-center">
					...
					<FeedbackCTA feedback={t("feedback")} cta={t("giveFeedback")} />
					...
					{upcoming.map((m) => (
						...
						>
							<div className="flex items-center justify-between">
								...
								<StatusBadge status={m.status as Status} label={t(`status.${m.status}`)} />
							</div>

							<h3 className="mt-4 text-lg font-medium">
								{translate(`milestones.${m.key}.title`)}
							</h3>
							...
							<p className="mt-3 text-sm text-zinc-600 dark:text-zinc-400">
								{translate(`milestones.${m.key}.description`)}
							</p>
									...
									.map((m) => (
										...
										>
											<div className="flex items-center justify-between">
												...
												<StatusBadge status="Shipped" label={t("status.Shipped")} />
											</div>
										...
										{translate(`milestones.${m.key}.title`)}
											...
											<div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
												{m.shippedAt && (
													...
													<span>
														{t("shippedDate", {
															date: formatShortDate((m as any)._date),
														})}
														</span>
													</span>
												)}
