...
export default async function Page({ params }: PageProps) {
	...
	return (
		<AppDetailShell appId={canonicalAppId} app={app} tocItems={[{ id: "usage", label: t("usageOverTime") }, { id: "model-leaderboard", label: t("modelLeaderboard") }]}>
			<div className="space-y-10">
			<section id="usage" className="scroll-mt-36 space-y-3">
				<div className="flex items-center gap-2">
					<Activity className="h-5 w-5 text-muted-foreground" />
					<h2 className="text-xl font-semibold leading-8">{t("usageOverTime")}</h2>
				</div>
				<AppUsageChart
					rows={resolvedRows}
			...
			<section id="model-leaderboard" className="scroll-mt-36 space-y-3">
				<div className="flex items-center gap-2">
					...
					<h2 className="text-xl font-semibold leading-8">{t("modelLeaderboard")}</h2>
				</div>
