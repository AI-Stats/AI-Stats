...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
	...
	return { title: messages.pages.settings, robots: { index: false, follow: false } };
}
...
export default async function SettingsLayout({
	...
	if (!initialData.signedIn) {
		...
		const safeReturnUrl = requestedPath.startsWith("/")
			? requestedPath
			: "/settings";
		redirect(`${localizeAuthPath(locale, "/sign-in")}?returnUrl=${encodeURIComponent(safeReturnUrl)}`);
	}
