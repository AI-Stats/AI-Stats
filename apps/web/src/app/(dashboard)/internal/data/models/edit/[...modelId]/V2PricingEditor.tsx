...
export default function V2PricingEditor({ modelId, focusProviderId }: { modelId: string; focusProviderId?: string }) {
	...
	return <div className="space-y-4 sm:space-y-5">
		<div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-start sm:justify-between">
			<div><h2 className="font-semibold">Database pricing</h2><p className="text-sm text-muted-foreground">Each offer is a versioned SKU. Prices below are USD per declared quantity.</p></div>
		...
		{drafts.map((draft, skuIndex) => {
			...
			return <section key={key} className="space-y-5 overflow-hidden rounded-xl border bg-background p-3 sm:p-5">
				...
				<div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
					...
					<label className="text-xs">Version<Input className="mt-1" type="number" min="1" value={draft.version} onChange={(e) => updateSku(skuIndex, { version: e.target.value })} /></label>
					<label className="text-xs">Currency<Input className="mt-1" maxLength={3} value={draft.currency} onChange={(e) => updateSku(skuIndex, { currency: e.target.value.toUpperCase() })} /></label>
					<label className="text-xs">Region<Select value={draft.region || "global"} onValueChange={(value) => updateSku(skuIndex, { region: value === "global" ? "" : value })}><SelectTrigger className="mt-1 h-10 w-full rounded-md sm:h-9"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="global">Global</SelectItem>{!currentRegionIsKnown ? <SelectItem value={draft.region}>{draft.region}</SelectItem> : null}{regionOptions.filter((region) => region.region_code !== "global").map((region) => <SelectItem key={region.region_code} value={region.region_code}>{region.display_name || region.region_code}</SelectItem>)}</SelectContent></Select></label>
					<label className="text-xs">Lifecycle<Select value={draft.status} onValueChange={(value) => updateSku(skuIndex, { status: value as SkuDraft["status"] })}><SelectTrigger className="mt-1 h-10 w-full rounded-md sm:h-9"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="active">Active</SelectItem><SelectItem value="deprecated">Deprecated</SelectItem><SelectItem value="disabled">Disabled</SelectItem>{draft.status === "draft" ? <SelectItem value="draft" disabled>Draft (legacy)</SelectItem> : null}</SelectContent></Select><span className="mt-1 block text-[10px] text-muted-foreground">Date state: {offerDateState(draft)}</span></label>
					<label className="text-xs">Effective from<Input className="mt-1" type="datetime-local" value={draft.effective_from} onChange={(e) => updateSku(skuIndex, { effective_from: e.target.value })} /></label>
					<label className="text-xs">Effective to<Input className="mt-1" type="datetime-local" value={draft.effective_to} onChange={(e) => updateSku(skuIndex, { effective_to: e.target.value })} /></label>
				</div>
					...
					{draft.meters.map((meter, meterIndex) => <div key={`${key}-${meterIndex}`} className="grid grid-cols-2 gap-3 rounded-lg border bg-muted/20 p-3 sm:grid-cols-3 xl:grid-cols-10">
						<label className="col-span-2 text-xs sm:col-span-1">Meter key<Select value={meter.meter_key} onValueChange={(value) => { const definition = source.meterDefinitions.find((item) => item.meter_key === value); updateMeter(skuIndex, meterIndex, definition ? { meter_key: definition.meter_key, display_label: definition.display_name, modality: definition.modality, direction: definition.direction ?? "", unit: definition.unit, unit_quantity: String(definition.default_unit_quantity) } : { meter_key: value }); }}><SelectTrigger className="mt-1 h-9 w-full rounded-md"><SelectValue /></SelectTrigger><SelectContent>{source.meterDefinitions.map((definition) => <SelectItem key={definition.meter_key} value={definition.meter_key}>{definition.display_name}</SelectItem>)}</SelectContent></Select></label>
						<label className="col-span-2 text-xs sm:col-span-1">Label<Input className="mt-1" value={meter.display_label} onChange={(e) => updateMeter(skuIndex, meterIndex, { display_label: e.target.value })} placeholder="Input tokens" /></label>
						<label className="text-xs">Modality<Input className="mt-1" value={meter.modality} onChange={(e) => updateMeter(skuIndex, meterIndex, { modality: e.target.value.toLowerCase() })} placeholder="text" /></label>
						<label className="text-xs">Direction<Select value={meter.direction || "none"} onValueChange={(value) => updateMeter(skuIndex, meterIndex, { direction: value === "none" ? "" : value as MeterDraft["direction"] })}><SelectTrigger className="mt-1 h-9 w-full rounded-md"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Neither</SelectItem><SelectItem value="input">Input</SelectItem><SelectItem value="output">Output</SelectItem></SelectContent></Select></label>
						<label className="text-xs">Unit<Input className="mt-1" value={meter.unit} onChange={(e) => updateMeter(skuIndex, meterIndex, { unit: e.target.value })} placeholder="token" /></label>
						<label className="text-xs">Quantity<Input className="mt-1" type="number" value={meter.unit_quantity} onChange={(e) => updateMeter(skuIndex, meterIndex, { unit_quantity: e.target.value })} placeholder="1000000" /></label>
						<label className="col-span-2 text-xs sm:col-span-1">Price (USD)<Input className="mt-1 font-mono" type="number" step="any" inputMode="decimal" value={meter.price_usd} onChange={(e) => updateMeter(skuIndex, meterIndex, { price_usd: e.target.value })} placeholder="0.00" /></label>
						<label className="text-xs">Display unit<Input className="mt-1" value={meter.display_unit} onChange={(e) => updateMeter(skuIndex, meterIndex, { display_unit: e.target.value })} placeholder="1M tokens" /></label>
						<label className="text-xs">Order<Input className="mt-1" type="number" min="0" value={meter.meter_order} onChange={(e) => updateMeter(skuIndex, meterIndex, { meter_order: e.target.value })} placeholder="100" /></label>
						<div className="col-span-2 flex items-end justify-end sm:col-span-1"><Button aria-label="Delete meter" className="w-full sm:w-9" type="button" variant="outline" size="icon" disabled={draft.meters.length === 1} onClick={() => updateSku(skuIndex, { meters: draft.meters.filter((_, index) => index !== meterIndex) })}><Trash2 className="h-4 w-4" /><span className="ml-2 sm:hidden">Delete meter</span></Button></div>
					</div>)}
				</div>
